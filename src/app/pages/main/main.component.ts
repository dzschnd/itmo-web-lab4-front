...
export class MainComponent implements AfterViewInit, OnInit {
  ...
  // form:
  public selectedX: number | null = null
  public readonly availableX = [-4, -3, -2, -1, 0, 1, 2, 3, 4]
  public selectedR: number | null = null
  ...
  isFormValid(form: NgForm) {
    const { x, y, r } = form.value;
    return x !== null && r !== null && y >= -3 && y <= 5 && /"^(?:-3|\+?5)(?:[.,]0{1,15})?$|^(?:-[210]|\+?[01234])(?:[.,]\d{1,15})?$/.test(y);
  }

  handleRChanged(event: any) {
    this.drawPlot()
  }
  ...
  drawPlot() {
    ...
    this.ctx.fillStyle = 'rgba(52,161,255,0.53)'
    this.ctx.strokeStyle = 'black'
    this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height)

    // rectangle
    this.ctx.fillRect(transformX(-this.R), transformY(0), this.R * this.scale, (this.R) * this.scale)
    ...
    // semicircle
    this.ctx.beginPath()
    this.ctx.arc(transformX(0), transformY(0), (this.R) * this.scale, 0, Math.PI / 2, false)
    this.ctx.arc(transformX(0), transformY(0), (this.R) * this.scale, 0, Math.PI / 2, false)
    this.ctx.lineTo(transformX(0), transformY(0))
    this.ctx.closePath()
    this.ctx.fill()
    ...
    //labels
    this.ctx.font = "25px 'Amatic SC', cursive";
    let labels = ['-R', '-R/2', 'R/2', 'R']
    const shotR = this.selectedR
    if (shotR != null) labels = [-shotR, -shotR / 2, shotR / 2, shotR].map(r => r.toString())
    this.ctx.fillStyle = 'black'
    const textSize = 5;
    this.ctx.fillText(labels[0], transformX(-this.R) - labels[0].length * textSize - 4, transformY(0) - 8)
    this.ctx.fillText(labels[1], transformX(-this.R / 2) - labels[1].length * textSize - 4, transformY(0) - 8)
    this.ctx.fillText(labels[2], transformX(this.R / 2) - labels[2].length * textSize - 4, transformY(0) - 8)
    this.ctx.fillText(labels[3], transformX(this.R) - labels[3].length * textSize - 4, transformY(0) - 8)
    this.ctx.fillText('x', transformX(this.R + 32) - 4, transformY(0) - 8)
    this.ctx.fillText(labels[0], transformX(0) + 4, transformY(-this.R) + 16)
    this.ctx.fillText(labels[1], transformX(0) + 4, transformY(-this.R / 2) + 16)
    this.ctx.fillText(labels[2], transformX(0) + 4, transformY(this.R / 2) + 16)
    this.ctx.fillText(labels[3], transformX(0) + 4, transformY(this.R) + 16)
    this.ctx.fillText('y', transformX(0) + 4, transformY(this.R + 32) + 16)

    // points
    this.shots.forEach(shot => {
      if (shot.r == this.selectedR)
  ...
  drawPoint(x: number, y: number, r: number, inArea: boolean) {
    ...
    this.ctx.fillStyle = inArea ? "#3fe124" : "red"
    this.ctx.fill()
    this.ctx.stroke()
  }
}
